...
training_param_spaces = {
    "xgboost": {"early_stopping_rounds": 20, "verbose": False},
    "catboost": {"early_stopping_rounds": 50, "logging_level": "Verbose"},
    "tabnet": {"batch_size": 256,},
}
...
def objective(trial):
    ...
    if args.model == "xgboost":
        if dataset.is_regression:
            ...
            model = XGBClassifier(
                **cfg_model, random_state=42, disable_default_eval_metric=True
            )
        ...
        model.fit(
            Xs["train"], ys["train"], eval_set=[(Xs["val"], ys["val"])], **cfg_training,
        )
        ...
        model.fit(
            Xs["train"], ys["train"], **cfg_training, eval_set=(Xs["val"], ys["val"]),
        )
