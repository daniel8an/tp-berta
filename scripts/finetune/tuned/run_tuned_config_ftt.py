...
def get_training_args():
    ...
    cfg = {
        ...
        "training": {"max_epoch": 500, "optimizer": "adamw",},
    }
...
if (
    dataset.X_num is None
):  # FT-Transformer official implementation requires at least one numerical features
    new_Xnum = {
...
for epoch in range(500):
    ...
    for batch in tqdm(train_loader, desc=f"epoch-{epoch}"):
        ...
        if tot_step % report_frequency == 0:
            print(
                f"\r(epoch) {epoch} (batch) {tot_step} (avg_loss) {tr_loss_holder.avg:.4f}",
                end="",
            )
    ...
    print(
        f"Epoch {epoch:03d} | Validation score: {val_score:.4f} | Test score: {test_score:.4f}"
    )
