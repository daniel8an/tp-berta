...
class TensorDictDataset(Dataset[TensorDict]):
    ...
    def __init__(self, tensor_dict: TensorDict) -> None:
        super().__init__()
        assert all(
            tensor_dict["input_ids"].size(0) == tensor.size(0)
...
def split(dataset: Dataset2, config: DataConfig = None):
    ...
    train_idx, val_idx = train_test_split(
        sample_idx, test_size=test_ratio, random_state=42, stratify=stratify
    )
    ...
    if config and config.pre_train or "test" in dataset.y:
        ...
        train_idx, val_idx = train_test_split(
            train_idx, test_size=test_ratio, random_state=42, stratify=stratify
        )
...
def data_preproc(
    dataset_name, config: DataConfig = None, no_str=False, tt: str = None
) -> Dataset2:
    assert os.path.exists(
        config.data_dir / f"{dataset_name}.csv"
    ), f"DATASET [{dataset_name}] not exists"
    if config is not None:
        dataset = Dataset2.from_csv(
            config.data_dir,
            dataset_name,
            max_cat_num=config.num_cat_token,
            no_str=no_str,
            tt=tt,
        )
    else:
        ...
        dataset = Dataset2.from_csv(
            DATA, dataset_name, max_cat_num=16, no_str=no_str, tt=tt
        )
    ...
    if config.preproc_type in ["ftt", "lm"]:
        ...
        cat_min_frequency = (
            0.05 if dataset.X_cat is not None else None
        )  # same as FTT settings
    ...
    cache_dir = (
        None
        if (config is None or not config.use_cache)
    ...
    prompt_infos = (
        dataset.prompt_infos
    )  # protect prompt infos for `transform_dataset` will read cache to overwrite it
    dataset = split(dataset, config)  # split into train, val, test
    dataset = transform_dataset(
        dataset, transformation, cache_dir
    )  # WARNING: cache will overwrite data split
    dataset = replace(dataset, prompt_infos=prompt_infos)
    return dataset


def encode_single_dataset(
    dataset: Dataset2, config: DataConfig,
):
    ...
    feature_names = {
        spl: [feature_name_map[v] for v in feature_names[spl]]
    ...
    def has_spl(spl):
        return any(
            spl in x
            for x in [dataset.y, dataset.X_num, dataset.X_str, dataset.X_cat]
            if x is not None
        )
    ...
    for spl in ["train", "val", "test"]:
        ...
        if num_fix_part is not None:
            for i, efn in enumerate(encoded_feature_names["num"]):
                ...
                if config.use_num_multiply:
                    num_input_scales.append(
                        ori_X_num[spl][:, i : i + 1].repeat(1 + len(efn), axis=1)
                    )
        ...
        if cat_fix_part is not None:
            for i in range(dataset.n_cat_features):
                ...
                assert (
                    cat_features.dtype == np.int64
                ), f"categorical feature should be long data type"
                ...
                each_part = np.repeat(
                    np.array([[cls_token_id] + efn + [mask_token_id]]),
                    repeats=N,
                    axis=0,
                )
                ...
                each_token_type = np.repeat(
                    np.array([[0] + [0] * len(efn) + [feature_type]]), repeats=N, axis=0
                )

                # position ids
                each_position_ids = np.repeat(
                    np.array([[0] + [i for i in range(1, len(efn) + 1)] + [0]]),
                    repeats=N,
                    axis=0,
                )

                # feature cls mask
                each_feature_mask = np.repeat(
                    np.array([[1] + [0] * len(efn) + [0]]), repeats=N, axis=0
                )
        ...
        if str_part is not None:
            for i in range(dataset.n_str_features):
                ...
                for j in range(N):
                    ...
                    # input ids
                    str_part[j].extend(
                        [cls_token_id] + efn + esv
                    )  # feature name + specific string value
                    # token types: string type (4)
                    str_token_types[j].extend([0] + [0] * len(efn) + [4] * len(esv))
                    # position ids
                    str_position_ids[j].extend(
                        [0] + [i for i in range(1, len(efn) + 1)] + [0] * len(esv)
                    )
                    # feature cls mask
                    str_feature_cls_mask[j].extend(
                        [1] + [0] * len(efn) + [0] * len(esv)
                    )

        # padding to max length
        num_fix_part_token = num_num_token + num_cat_token
        max_str_tokens = (
            config.max_seq_length - num_fix_part_token - 2
        )  # 2: [cls], [sep]
        max_str_tokens = min(
            max([len(str_tokens) for str_tokens in str_part]), max_str_tokens
        )
        ...
        for i in range(N):
            ...
            str_part[i] += [sep_token_id] + [pad_token_id] * max(
                0, pad_length
            )  # [x] [sep] [pad] ...
        ...
        input_ids = np.concatenate(
            ...
            + [
                token
                for token in [num_fix_part, cat_fix_part, str_part]
        ...
        token_type_ids = np.concatenate(
            ...
            + [
                token_type
                for token_type in [num_token_types, cat_token_types, str_token_types]
        ...
        position_ids = np.concatenate(
            ...
            + [
                pos_ids
                for pos_ids in [num_position_ids, cat_position_ids, str_position_ids]
        ...
        feature_cls_mask = np.concatenate(
            ...
            + [
                x
                for x in [
                    num_feature_cls_mask,
                    cat_feature_cls_mask,
                    str_feature_cls_mask,
                ]
                if x is not None
            ],
            axis=1,
        ...
        input_scales = np.concatenate(
            ...
            + [
                x
                for x in [num_input_scales, cat_input_scales, str_input_scales]
                if x is not None
            ],
            axis=1,
        )
