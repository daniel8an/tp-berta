...
class Dataset2:
    ...
    def from_csv(
        ...
        if (
            len(X_str_idx) > 0
        ):  # for LM baselines (e.g., TP-BERTa), all categorical features are treated as strings
            feature_names["str"] = feat_names[X_str_idx].tolist()
            X_str = {
                "train": df.iloc[:, X_str_idx].values.astype(np.str).astype(np.object_)
            }
...
def cat_process_nans(X: ArrayDict, policy: Optional[CatNanPolicy]) -> ArrayDict:
    ...
    nan_masks = {
        k: v == CAT_MISSING_VALUE for k, v in X.items()
    }  # can't perform elementwise comparison if v is not 'str/object' type
...
def transform_dataset(
    dataset: Dataset2, transformations: Transformations, cache_dir: Optional[Path],
) -> Dataset2:
    ...
    if cache_dir is not None:
        ...
        cache_path = (
            cache_dir
            / f"cache__{dataset.name}__{transformations_str}__{transformations_md5}.pickle"
        )
